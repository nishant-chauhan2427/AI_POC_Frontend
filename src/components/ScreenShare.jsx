...
const ScreenShare = forwardRef(function ScreenShare(
  ...
  const uploadRecording = useCallback(
    async (chunksToUpload) => {
      ...
      try {
        console.log(
          "🎥 Starting screen recording upload with",
          chunks.length,
          "chunks"
        );
  ...
  useImperativeHandle(ref, () => ({
    stopAndUpload: async () => {
      ...
      if (isRecording) {
        ...
        console.log(
          "✓ Recording stopped, chunksRef.current.length:",
      ...
      if (chunksRef.current.length > 0) {
        console.log(
          "📤 Starting automatic upload with",
          chunksRef.current.length,
          "chunks"
        );
        try {
  ...
  return (
    <div style={styles.card}>
      ...
      >
        {isRecording ? "Stop Recording" : "Initiate Screen and Audio Recording"}
      </button>
