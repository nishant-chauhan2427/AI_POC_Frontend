...
export default function QuestionBox({
  ...
  console.log(question, "question123");
  ...
  async function getTranscript(blob) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errorData.detail || errorData.message || `Server error: ${res.status}`
        );
      }
  ...
  async function handleMarkForReview() {
    ...
    try {
      ...
      params.append("question_id", String(question?._id));
      params.append("question", String(question?.question));
      ...
      if (!res.ok) {
        ...
        if (res.status === 400) {
          ...
          form.append("question_id", String(question?._id));
          form.append("question", String(question?.question));
          if (sessionId) form.append("session_id", String(sessionId));
          console.log(
            "mark_review payload (multipart): candidate_id, question_id"
          );
          const res2 = await fetch(`${API_BASE}/questions/mark_review`, {
            method: "POST",
            body: form,
          });
          const ct2 = res2.headers.get("content-type") || "";
          const payload2 = ct2.includes("application/json")
            ? await res2.json()
            : await res2.text();
          if (!res2.ok) {
            const msg2 =
              (payload2 &&
                (payload2.detail || payload2.message || payload2.error)) ||
              `Failed to mark for review (${res2.status})`;
          ...
          const msg =
            ...
            `Failed to mark for review (${res.status})`;
          throw new Error(msg);
        }
      }
    } catch (e) {
  ...
  async function handleSkip() {
    ...
    try {
      ...
      if (!res.ok) {
        const msg =
          (payload && (payload.detail || payload.message || payload.error)) ||
          `Failed to skip question (${res.status})`;
        throw new Error(msg);
      }
  ...
  async function submitAnswer(audioBlob) {
    ...
    setError("");
    console.log(question, "expected ans12");
    try {
      const formData = new FormData();
      formData.append("candidate_id", candidateId);
      formData.append("question_id", question?._id);
      ...
      const audioFile = new File(
        [audioBlob],
        `recording_${candidateId}_q${question?._id}.webm`,
        {
          type: "audio/webm;codecs=opus",
        }
      );
      ...
      for (let [key, value] of formData.entries()) {
        console.log(
          `${key}:`,
          key === "audio_file"
            ? `File(${value.size} bytes, ${value.type})`
            : value
        );
      }
  ...
  async function handleFinish() {
    ...
    try {
      ...
      const data = await getJSON(
        `/questions/get_review_questions?candidate_id=${encodeURIComponent(
          candidateId
        )}`
      );
  ...
  const styles = {
    ...
    glow: {
      ...
      borderRadius: 16,
      background:
        "linear-gradient(135deg, rgba(99,102,241,0.12), rgba(168,85,247,0.12), rgba(59,130,246,0.12))",
      filter: "blur(24px)",
    },
    card: {
      position: "relative",
    ...
    badgeState: (active) => ({
      ...
      border: `1px solid ${
        active ? "rgba(248,113,113,0.4)" : "rgba(52,211,153,0.4)"
      }`,
  ...
  const getStatusBadge = () => {
    ...
    if (isSpeaking) return { text: "Speaking...", active: true };
      ...
      <div style={styles.card}>
        <div style={styles.header}>
          <div style={styles.badges}>
            ...
            <span style={styles.badgeState(statusBadge.active)}>
              {statusBadge.text}
            </span>
